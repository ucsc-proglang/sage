...
let p6_pre = (Cons (List Int) p2 (Cons (List Int) p3 (Null (List Int) unit)));;
let p6 = flatten Int p6_pre;;
let funct = (fn (x:Int) => if [Bool] (x > 4) then true else false);;
let p7 = map Int Bool funct p6;;
let p8 = rev_map Int Bool funct p5;;

/* Length List */
let GTInt (lb:Int) :* = (x:Int.(x>lb));;

datatype LList (T:*) (n:Int) =
  LNull of (u:(y:Unit.(n=0)))
| LCons of (x:T) (r:(LList T (n-1)));;

let rec l_length (T:*) (n:Int)(l:LList T n):Int = 
      caseLList T n l Int
      (fn u => 0)
      (fn x r => (1 + (l_length T (n-1) r)));;

let l_hd (T:*) (n:(GTInt 0)) (l:(LList T n)) : T =
      caseLList T n l T
      (fn u => (fail T)) 
      (fn x r => x);;

let l_tl (T:*)(n:Int) (l:(LList T n)) : (LList T (n-1)) =
      caseLList T n l (LList T (n-1))
      (fn u => (fail (LList T (n-1))))
      (fn x r => r);;

let rec l_nth (T:*) (n:Int) (l:(LList T n)) (i:Int): T =
      caseLList T n l T
      (fn u => (fail T))
      (fn x r => 
         (if [T] (i = 0) 
         then x
         else (l_nth T (n-1) r (i-1))));;


let rec append (T:*) (n:Int) (l1:(LList T n)) (m:Int) (l2:(LList T m)) : 
      (LList T (n+m))= 
      caseLList T n l1 (LList T (n+m))
      (fn u => l2)
      (fn x r => (LCons T (n+m) x (append T (n-1) r m l2)));;


let rec l_map (S:*) (T:*) (n:Int) (f:(S->T)) (l:(LList S n)) :(LList T n) =
      caseLList S n l (LList T n)
      (fn u => (LNull T n unit))
      (fn x r => (LCons T n (f x) (l_map S T (n-1) f r)));;


let rec l_split (S:*) (T:*) (n:Int) (pl:(LList (Pair S T) n)) : 
      (Pair (LList S n) (LList T n)) =
      caseLList (Pair S T) n pl (Pair (LList S n) (LList T n))
      (fn u => (Pcons (LList S n) (LList T n) (LNull S n unit) (LNull T n unit)))
      (fn x r => 
          (casePair (LList S (n-1)) (LList T (n-1)) (l_split S T (n-1) r) 
                (Pair (LList S n) (LList T n))
          (fn ls lt => 
             (casePair S T x (Pair (LList S n) (LList T n))
             (fn s t => 
                (Pcons (LList S n) (LList T n) 
                (LCons S n s ls) (LCons T n t lt)))))));; 

let rec l_combine (S:*) (T:*) (n:Int) (l1:(LList S n)) (l2:(LList T n)) : 
      (LList (Pair S T) n) =
      caseLList S n l1 (LList (Pair S T) n)
      (fn u1 => 
          (caseLList T n l2 (LList (Pair S T) n)
          (fn u => (LNull (Pair S T) n unit))
          (fn x r => (fail (LList (Pair S T) n)))))
      (fn x1 r1 =>
          (caseLList T n l2 (LList (Pair S T) n)
          (fn u2 => (fail (LList (Pair S T) n)))
          (fn x2 r2 => (LCons (Pair S T) n (Pcons S T x1 x2) 
                (l_combine S T (n-1) r1 r2)))));; 


let rec htc_find (T:*) (pred:(T->Bool)) (l:(List T)) : (x:T.(pred x)) =
      caseList T l {x:T|pred x}
      (fn u => (fail (x:T.(pred x))))
      (fn x r => (if [T] (pred x)
                 then x
                 else (htc_find T pred r)));;


let rec htc_filter (T:*) (pred:(T->Bool)) (l:(List T)) 
      : {x:List T|for_all T pred x} =
      caseList T l {x:List T|for_all T pred x}
      (fn u => (Null T unit))
      (fn x r =>  (if [List T] (pred x)
                 then (Cons T x (htc_filter T pred r))
                 else (htc_filter T pred r)));;

let rec htc_partition (T:*) (pred:(T->Bool)) (l:(List T)) : 
      (Pair (x:(List T).(for_all T pred x)) 
      (y:(List T).(for_all T (fn (z:T) => (not(pred z))) y))) =
      caseList T l (Pair (List T) (List T))
      (fn u => (Pcons (List T) (List T) (Null T unit) (Null T unit)))
      (fn x r => 
          (casePair (List T) (List T) (htc_partition T pred r) 
          (Pair (List T) (List T))
          (fn l1 l2 =>
                (if [(Pair (List T) (List T))] (pred x)
                 then (Pcons (List T) (List T) (Cons T x l1) l2)
