...
let same_type_term (tm:Term) : * =
	(t : Term .  (typeeq (typeof emptyctx tm) (typeof emptyctx t)))
;;
...
let rec eval1 (tm:Term) : (same_type_term tm) =
	/* Unless I put crazy refinements on the subst
		and shift operations, this is necessary for
		the program to be well-typed */
  let return_type =  Dynamic in 
